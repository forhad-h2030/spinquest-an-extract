...
MODE = "argmax"   # "argmax", "prob", or "second_max"
...
# class, all ANDed together on top of argmax==k. Same values as
# corrected_an.py's PROB_CUTS.
...
SECOND_MAX_VETO = 0.2   # MODE="second_max" only: veto jpsi/psip/dy to comb
                        # if any OTHER class's prob exceeds this
...
def main():
    ...
    if MODE == "argmax":
        ...
        # unlike corrected_an.py, fold anything that fails its own class's
        # prob cut into comb here -- for a mass overlay, jpsi+psip+dy+comb
        # should still sum to the total so no events silently vanish
        ...
        # same leftover-to-comb folding as MODE="prob"
        comb_mask = comb_mask | (qual_mask & ~jpsi_mask & ~psip_mask & ~dy_mask)
